...
/** 组件挂载时从后端获取当前 HTTP Server 配置 */
onMounted(async () => {
  try {
    const config = await invoke<{ http_port: number; auto_connect: boolean }>("get_server_config");
...
async function savePort() {
  try {
    ...
    showToast(t("app.toast.portSaved"), "success");
  } catch (err) {
    showToast(t("app.toast.portSaveFailed") + ": " + String(err), "error");
  }
}
    ...
    <aside class="sidebar">
      ...
      <div class="server-status">
        <div class="server-status-header">
          ...
          <span>{{ t("app.http.title") }}</span>
        </div>
        <div class="server-status-body">
          <div class="form-group">
            <label for="server-port">{{ t("app.http.port") }}</label>
            <input
              id="server-port"
              type="number"
          ...
          <button class="save-port-btn" @click="savePort">{{ t("app.http.save") }}</button>
          <div class="server-url">
            {{ t("app.http.address") }}: <code>http://localhost:{{ serverPort }}/mcp</code>
          </div>
        </div>
      </div>
    </aside>
...
.server-status-body .form-group {
  margin-bottom: 0;
}

.server-status-body .form-group label {
  display: block;
  font-size: 11px;
  color: #8b949e;
  margin-bottom: 4px;
...
.server-status-body .form-group input {
  width: 100%;
  padding: 7px 10px;
  font-size: 12px;
  background: #0d1117;
...
.server-status-body .form-group input:focus {
  border-color: #58a6ff;
}
